...
function Teachers() {
    ...
    useEffect(() => {
        axios
            .get('https://k4knowledgegame.pythonanywhere.com/api/statisticTeachers/')
            .then((res) => {
    ...
    const handleUpdate = async (e) => {
        if (window.confirm(`Bạn có chắc chắn muốn sửa trường dữ liệu ${formData.idTransactionHistory} không?`)) {
            await axios.put('https://k4knowledgegame.pythonanywhere.com/api/statisticTeachers/', formData);
        } else {
            e.preventDefault();
        }
    };
