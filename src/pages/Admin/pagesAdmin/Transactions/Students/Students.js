...
function Students() {
    ...
    const handleUpdate = async (e) => {
        if (window.confirm(`Bạn có chắc chắn muốn sửa trường dữ liệu ${formData.idTransactionHistory} không?`)) {
            await axios.put('https://k4knowledgegame.pythonanywhere.com/api/statisticStudents/', formData);
        } else {
            e.preventDefault();
        }
    };
    ...
    const handleAdd = async (e) => {
        ...
        await axios.post('https://k4knowledgegame.pythonanywhere.com/api/statisticTeachers/', formData);
    };
    return (
        <div>
