...
function Titles() {
    ...
    useEffect(() => {
        axios
            .get('https://k4knowledgegame.pythonanywhere.com/api/titleContentAll/')
            .then((res) => {
                setTitles(res.data);
            })
            .catch((error) => {
    ...
    const handleUpdate = async (e) => {
        if (window.confirm(`Bạn có chắc chắn muốn sửa trường dữ liệu ${formData.idTitleContent} không?`)) {
            await axios.put('https://k4knowledgegame.pythonanywhere.com/api/titleContentAll/', formData);
        } else {
            e.preventDefault();
        }
    };
    ...
    const handleAdd = async (e) => {
        ...
        await axios.post('https://k4knowledgegame.pythonanywhere.com/api/titleContentAll/', formData);
    };
    return (
        <div>
