...
function Courses() {
    ...
    const handleUpdate = async (e) => {
        if (window.confirm(`Bạn có chắc chắn muốn sửa trường dữ liệu ${formData.idCourse} không?`)) {
            await axios.put('https://k4knowledgegame.pythonanywhere.com/api/course/', formData);
            e.preventDefault();
        } else {
            e.preventDefault();
        }
    ...
    const handleAdd = async (e) => {
        ...
        await axios.post('https://k4knowledgegame.pythonanywhere.com/api/course/', formData);
    };
    return (
        <div>
